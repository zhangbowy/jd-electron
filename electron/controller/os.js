...
class OsController extends Controller {
  ...
  async getDewu() {
    ...
    const p = {
      ...
      const res = await hc.request("https://app.dewu.com/api/v1/h5/pc/index/fire/flow/product/detailV3", {
        "headers": {
          "accept": "*/*",
  ...
  async getDewuSpuByUrl(url) {
    // url = "https://dw4.co/t/A/1oggWBQoT"
    const hc = new HttpClient();

    const res = await hc.request(url, {
      "headers": {
        "accept": "*/*",
        "accept-language": "zh,en-US;q=0.9,en;q=0.8,zh-CN;q=0.7",
        "appid": "h5",
  ...
  async checkLogin(thread = 0) {
    ...
    thread = this.jdb.getItem('thread') || 0
  ...
  async syncJdToken(thread) {
    const cookies = await this.getCodeCookie()
    const hc = new HttpClient();
    const res = await hc.request("http://122.51.6.129:7001/saveCookies", {
      "headers": {
        "content-type": "application/json;charset=UTF-8",
        "accept": "*/*",
        "accept-language": "zh-CN,zh;q=0.9,en;q=0.8",
    ...
    this.jdb.setItem('thread', res.data.thread)
    console.log(res.data.thread, 'syncJdToken')
  }

  async syncJdBeinanToken(thread) {
    const cookies = await this.getCodeCookie()
    const hc = new HttpClient();
    const res = await hc.request("http://118.89.164.55:7001/saveCookies", {
      "headers": {
        "content-type": "application/json;charset=UTF-8",
        "accept": "*/*",
        "accept-language": "zh-CN,zh;q=0.9,en;q=0.8",
